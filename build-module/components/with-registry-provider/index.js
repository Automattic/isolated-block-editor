...
const withRegistryProvider = createHigherOrderComponent(WrappedComponent => withRegistry(props => {
  ...
  useEffect(() => {
    ...
    const store = newRegistry.registerStore(STORE_NAME, storeConfig(preferencesKey, defaultPreferences)); // Create the core/block-editor store separatley as we need the persistence plugin to be active

    const blockEditorStore = newRegistry.registerStore('core/block-editor', { ...blockEditorStoreConfig,
      persist: ['preferences']
    }); // Duplicate the core/editor store so we can decorate it

    const editorStore = newRegistry.registerStore('core/editor', { ...coreEditorStoreConfig,
      selectors: { ...coreEditorStoreConfig.selectors,
